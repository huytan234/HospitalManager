...
namespace QL_BenhVien
{
    public partial class FrmQuanLyBenhNhan : Form
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("DELETE FROM BenhNhan WHERE id = @idBN;", _conn.connection());
        ...
        private void btn_editBN_Click(object sender, EventArgs e)
        {
            using (HOSPITALDBEntities1 context = new HOSPITALDBEntities1())
            {
                ...
                if (benhNhan != null)
                {
                    ...
                    {
                        DialogResult dialogResult = MessageBox.Show("Bạn có chắc chắn muốn sửa bệnh nhân này?", "Xác nhận xóa", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                        if (dialogResult == DialogResult.Yes)
                        {
        ...
        private void btnTimTheoGT_Click(object sender, EventArgs e)
        {
